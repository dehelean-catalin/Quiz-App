...
function QuizAnswer(props) {
	...
	sessionStorage.setItem("score", JSON.stringify({ goodPoints: goodPoints, badPoints: badPoints }));
	const [counter, setCounter] = useState(15);
	const [pause, setPause] = useState(true);

	useEffect(() => {
		counter >= 1 && !props.disableInput && props.next && pause && setTimeout(() => setCounter(counter - 1), 1000);
	...
	useEffect(() => {
		if (badPoints > goodPoints) {
			props.callback(true);
		} else if (badPoints <= goodPoints && props.disableInput && disableConfirm) {
			props.callback(false);
		} else {
			props.callback(true);
		}
	...
	return (
		<div className="answer-container">
				...
				{props.answers &&
					props.answers.map((value, index) => (
						<button
							...
							style={
								props.disableInput && answerValue === value && answerValue === props.correct_answer
									? { backgroundColor: "#D4EDDA" }
									: props.disableInput && answerValue !== value && value === props.correct_answer
									? { backgroundColor: "#D4EDDA" }
									: props.disableInput && answerValue === value && answerValue !== props.correct_answer
			...
			<div className="confirm-btn-wrapper">
				<div className="score">
					Score: <span style={goodPoints - badPoints > 0 ? { color: "green" } : {}}>{goodPoints - badPoints}</span>
				</div>
				<button
			...
			<div
				className={
					props.disableInput && answerValue === props.correct_answer ? `brilliant-point` : `hiddend-brilliant-point`
				}
			>
				{" "}
				+1 Brilliant point
			...
			<div
				className={
					props.disableInput && answerValue !== props.correct_answer ? `dummy-point` : `hiddend-brilliant-point`
				}
			>
				{" "}
				+1 dummy point
			</div>
			<div style={{ position: "absolute", color: "black" }}>lala {props.correct_answer}</div>
